...
weatherForm.addEventListener("submit", (eve) => {
  ...
  if (postTitle.value) {
    ...
    const prevSearchList = document.createElement("ul");
    searchHistory.append(prevSearchList);

    const lastSearch = document.createElement("li");
    const lastSearchAnchor = document.createElement("a");
    lastSearchAnchor.innerText = postTitle.value;
    lastSearch.append(lastSearchAnchor);

    lastSearchAnchor.addEventListener("click", (eve) => {
      eve.preventDefault();
      ...
      fetch(url)
        ...
        .then((res) => {
          ...
          mainContent.innerHTML = `<h2 style="color: blue">${eve.target.innerText}</h2>`;
