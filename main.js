...
const form = document.querySelector(`form`)
 form.addEventListener(`submit`, (event) => {
    ...
    const locationId = `${event.target.search.value}`
    ...
    .then(res => {
        ...
        //Adding current search to previous search list
        // const ul = document.querySelector(`.searches ul`)
        // const prev = document.createElement(`li`)
        // prev.innerHTML = `<a href='${BASE_URL}${locationId}${json}>${res.nearest_area[0].areaName[0].value}</a> <span>${res.current_condition[0].FeelsLikeF}°F</span>`
        // ul.append(prev)

        const prev = document.querySelector(`.searches a`)
        prev.setAttribute(`href`, `${BASE_URL}${locationId}${json}`)
